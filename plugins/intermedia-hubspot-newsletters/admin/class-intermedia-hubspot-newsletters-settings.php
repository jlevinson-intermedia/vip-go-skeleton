...
class Intermedia_Hubspot_Newsletters_Admin_Settings {
	...
	public function setup_plugin_options_menu() {
		...
		$allow = esc_attr( get_user_meta( $current_user->ID, 'hubspot_newsletter_plugin_access', true ) );
		
		$user_roles = $current_user->roles;
		$user_role = strtolower(array_shift($user_roles));
		*/
		
		//if($allow == 'allow' || $user_role == 'administrator') {
			...
			add_plugins_page(
				...
				'edit_others_posts',									// Which type of users can see this menu item
	...
	public function initialize_intermedia_hubspot_newsletters_newsletters_settings() {
		...
		if( !isset( $registered_crops ) || empty( $registered_crops ) ){

			$registered_crops = array(
				'full',
				'thumbnail',
				'medium',
				'medium_large',
