...
class Intermedia_newsletters_Entities {
        ...
        if ( isset( $entities_options['cpt_included'] ) && $entities_options['cpt_included'] !== '' ) {
            ...
            if( current_user_can('manage_options') ) {
                ...
                add_meta_box(
                    ...
                    'entities_newsletter_image', // metabox ID
                    'Newsletter Image', // title
                    array( $this, 'upload_newsletter_image_metabox_callback' ),
                    $entities_options['cpt_included'], // post type or post types in array
                    'normal',
                    'default'
    ...
    public function upload_newsletter_image_metabox_callback( $post_object ) {
        $newsletter_image_url = get_post_meta( $post_object->ID, 'entities_newsletter_image', true );
        ob_start(); ?>
        <p class="entities-box">
            <label for="entities_image">Image for newsletter:</label>
            <input type="text" name="entities_newsletter_image" style="min-width:500px;max-width:100%;padding:5px;" value="<?php echo $newsletter_image_url; ?>" />
        </p>
    ...
    public function entities_save_metaboxdata( $post_id, $post ) {
        ...
        if ( isset( $entities_options['cpt_included'] ) && in_array( $post->post_type, $entities_options['cpt_included'] ) ) {
            ...
            if( current_user_can('manage_options') && isset($_POST['entities_newsletter_image']) ) {
                update_post_meta( $post_id, 'entities_newsletter_image', sanitize_text_field( $_POST['entities_newsletter_image'] ) );
            }
